...
import MenuVistaAnalytics from '../analytics/tracking';
...
export default function SababaFalafelPage() {
  ...
  useEffect(() => {
    ...
    MenuVistaAnalytics.init('sababa-falafel');
    
    // Clean up on component unmount
    return () => {
      if (MenuVistaAnalytics.currentViewItem) {
        MenuVistaAnalytics.trackItemViewEnd(MenuVistaAnalytics.currentViewItem);
      }
    };
  }, []);
  ...
  const handleItemClick = (item) => {
    ...
    MenuVistaAnalytics.trackItemClick(item.id, activeTab);
  };
